...
variable "load_balancer_configurations" {
  description = <<-EOT
    List of load balancer configurations for the ECS service.
    If empty and use_alb=true, a default configuration will be created automatically using container_port, health_check_path, and aws_lb_listener_rule_priority.
    This means you don't need to configure this for simple single load balancer setups.
  EOT
  type = list(object({
